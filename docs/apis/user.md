...
返回可开票的订单列表（已支付、未取消、无有效退款，且未被其他待处理/已批准发票申请关联）。

> **不可开票的订单**：未支付（`pending`）、已取消（`canceled`）、存在有效退款（进行中或已完成）的订单均不可开票，
> 提交申请时同样会被拒绝（见「提交发票申请」）。退款被拒绝/取消后订单恢复可开票。
> 券订单的开票金额为**实付口径**（商品金额 + 运费 − 优惠券抵扣）。
...
> **订单校验**：传入 `order_ids` 时逐单校验，以下情况拒绝申请（错误信息为对应提示）：
> 订单不属于当前用户 / 订单与申请租户不一致 / **订单未支付或已取消** / **订单已申请退款（进行中或已完成）** / 订单已申请过开票。
> 开票金额由关联订单的实付金额（商品金额 + 运费 − 优惠券抵扣）累加得出。
