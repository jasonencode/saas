...
- 资源创建成功（如创建订单、申请退款、评价商品等）返回 HTTP 201，响应体为新建资源（无 `code`/`message`/`data` 包裹）
...
```json
[
    { "tenant_id": 1, "no": "202501010001", "total_amount": "198.00" },
    { "tenant_id": 2, "no": "202501010002", "total_amount": "99.00" }
]
```
