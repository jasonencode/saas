...
获取登录所需的图形验证码（图片 base64 和标识 Key），支持多种验证码类型。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| type | string | 否 | 验证码类型，默认 `default`，可选值见下表 |

**type 可选值**

| 值 | 说明 |
|----|------|
| default | 字母数字混合，4 位，区分度干扰线 |
| math | 数学算式（加/减/乘） |
| number | 纯数字，4 位 |
| flat | 扁平风格，6 位，浅色背景 |
| mini | 迷你版，3 位小图（60×32） |
| inverse | 高难度，5 位，反色/模糊/锐化 |
| admin | 数学算式，管理端登录用 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| type | string | 实际返回的验证码类型 |
| key | string | 验证码标识 Key，提交登录/发短信时需要回传 |
| img | string | 验证码图片的 base64 数据（JPEG） |
