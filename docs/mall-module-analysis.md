...
> **分析时间**: 2025-07-16
> **复查时间**: 2026-06-15（全面检查）, 2026-06-16（问题复盘）
> **分析范围**:
...
| #      | 文件                                | 问题描述                                                                                   | 状态    |
|--------|-----------------------------------|----------------------------------------------------------------------------------------|-------|
| **C1** | `OrderService.php:270-291`        | `cancel()` 事件分发时机错误：`OrderCanceled::dispatch` 在 `$order->save()` 之前调用，事件监听器读取数据库状态会不一致 | ✅ 已修复 |
| **C2** | `OrderService.php:511-523`        | `delete()` 在 soft delete 之后写日志，且复用 `cancel` 的状态验证逻辑，已完成订单无法删除                          | ✅ 已修复 |
| **C3** | `OrderService.php:396-408`        | `deliver()` 变量命名误导：`$express` 实际是 `OrderShipping` 类型，与快递公司 ID 混淆                       | ✅ 已修复 |
| **C4** | `ExpressesRelationManager.php:18` | 关联名称不匹配：使用 `'expresses'` 但 Order 模型定义的是 `shippings()`                                  | ✅ 已修复 |
| **C5** | `OrderInfolist.php:93-105`        | 引用不存在的关联 `expresses.express.name`，物流信息无法显示                                             | ✅ 已修复 |
| **C6** | `ProductsTable.php:30`            | 使用 `categories.name` 但 Product 关联是单数 `category()`                                      | ✅ 已修复 |
| **C7** | `OrderController.php:34-37`       | SQL 注入风险：`$request->keyword` 直接拼接到 LIKE 查询，`%` 和 `_` 通配符未转义                            | ✅ 已修复 |
| **C8** | `OrderController.php:74`          | `Sku::find()` 可能返回 null，传入 `OrderItemDto::make()` 会报错                                  | ✅ 已修复 |
| **C9** | `ProductCollection.php:21`        | 引用不存在的属性 `$item->sales`，应为 `$item->total_sale`                                         | ✅ 已修复 |

### 3.2 🟡 N+1 查询问题

| #      | 文件                            | 问题描述                                                                       | 状态      |
|--------|-------------------------------|----------------------------------------------------------------------------|---------|
| **N1** | `Product.php:108-167`         | 4 个 accessor 通过 `$this->skus()->sum()` 访问 SKU，列表展示时每个 Product 触发 4-5 次额外查询 | ✅ 已修复   |
| **N2** | `IndexController.php:32-35`   | 缺少 `skus` 预加载，ProductCollection 渲染时触发 N+1                                  | ✅ 已修复   |
| **N3** | `ProductController.php:19-20` | 同 N2，缺少 `skus` 预加载                                                         | ✅ 已修复   |
| **N4** | `StatsOverview.php:19-58`     | 6 个独立 count 查询，可合并优化                                                       | ⚠️ 缓存优化 |
| **N5** | `CartItemResource.php:26`     | `is_available` 字段可能触发 N+1                                                  | ✅ 已预加载  |

### 3.3 🟠 缺失状态检查

| #      | 文件                         | 问题描述                            | 状态    |
|--------|----------------------------|---------------------------------|-------|
| **S1** | `RefundService.php:28-37`  | `refunded()` 缺少状态验证，重复调用会重复回退库存 | ✅ 已修复 |
| **S2** | `RefundService.php`        | 缺少退款完成/失败事件分发                   | ✅ 已修复 |
| **S3** | `CartService.php:156-183`  | `addItem()` 缺少商品上架状态检查          | ✅ 已修复 |
| **S4** | `CartService.php:20-33`    | `updateItemQty()` 缺少商品上架状态检查    | ✅ 已修复 |
| **S5** | `OrderService.php:390`     | `deliver()` 未验证 items 是否已发货     | ✅ 已修复 |
| **S6** | `ProductService.php:14-25` | `audit()` 缺少状态转换验证              | ✅ 已修复 |
| **S7** | `ProductService.php:30-41` | `up()/down()` 缺少前置状态验证          | ✅ 已修复 |

### 3.4 🟢 潜在 Bug 和边界问题

| #       | 文件                            | 问题描述                                         | 状态    |
|---------|-------------------------------|----------------------------------------------|-------|
| **B1**  | `Product.php:56-62`           | `saved` 事件会记录 created 事件，日志包含不必要的初始数据        | ✅ 已修复 |
| **B2**  | `Delivery.php:37-43`          | `saving` 事件每次保存都触发查询，即使 `is_default` 未变化     | ✅ 已修复 |
| **B3**  | `ReturnAddress.php:31-37`     | 同 B2                                         | ✅ 已修复 |
| **B4**  | `OrderService.php:511-523`    | 软删除后写日志，可能受全局 scope 影响                       | ✅ 无影响 |
| **B5**  | `CartService.php:106-129`     | `mergeSessionCart()` 缺少事务保护                  | ✅ 已修复 |
| **B6**  | `DeliveryService.php:84`      | `property_exists()` 不可靠，应使用 `getAttribute()` | ✅ 已修复 |
| **B7**  | `Cart.php:67-69`              | `isExpired()` 可能触发 N+1                       | ✅ 无影响 |
| **B8**  | `CartController.php:214`      | `clear()` 后重新加载空 items，无意义查询                 | ✅ 已修复 |
| **B9**  | `DeliveryService.php:158-169` | `ceilDivision()` 除数为 0 时会报错                  | ✅ 已修复 |
| **B10** | `OrderService.php:347,602`    | `assertCan` 在事务外执行，存在竞态条件                    | ✅ 已修复 |

### 3.5 💡 代码质量问题

| #      | 文件                          | 问题描述                                  | 状态      |
|--------|-----------------------------|---------------------------------------|---------|
| **Q1** | `OrderCollection.php:10-15` | `toArray()` 返回空数组，未完成实现               | ✅ 已修复   |
| **Q2** | `OrderItemDto.php:71-74`    | `getFreight()` 始终返回 '0.00'，未完成实现      | ⚠️ 待实现  |
| **Q3** | `CartService.php:175-179`   | CartItem 创建时缺少 `product_id` 字段        | ✅ 已修复   |
| **Q4** | `StoreService.php:41`       | 比较逻辑冗余，可简化为 enum 转换                   | ⚠️ 低优先级 |
| **Q5** | `RefundInfolist.php:15-16`  | 标签中英文混用不一致                            | ✅ 已修复   |
| **Q6** | `RefundItem.php`            | 缺少 `qty` 属性显式定义                       | ✅ 已修复   |
| **Q7** | `ProductCategory.php:24-26` | 全局 scope 无移除方法                        | ✅ 已修复   |
| **Q8** | `DeliveryService.php:53-66` | 规则匹配不支持降级（district → city → province） | ✅ 已修复   |

### 3.6 📋 枚举和事件问题

| #      | 文件                       | 问题描述                                      | 状态    |
|--------|--------------------------|-------------------------------------------|-------|
| **E1** | `OrderStatus.php:33`     | `PartiallyShipped` 值为 `'partially'` 不具描述性 | ✅ 已修复 |
| **E2** | `RefundScopes.php:29-32` | `ofPending()` 包含 Processing 状态，语义不符       | ✅ 已修复 |
| **E3** | `Events/Mall/Order*.php` | 8 个 Order 事件子类都是空类                        | ❌     |
| **E4** | `RefundBaseEvent.php:14` | `protected` 可见性与 OrderBaseEvent 风格不一致     | ✅ 已修复 |

### 3.7 🖥️ Filament 后台问题

| #      | 文件                        | 问题描述                         | 状态 |
|--------|---------------------------|------------------------------|----|
| **F1** | `OrderResource.php:46-52` | 缺少 Edit 页面，但 Table 有编辑操作     | ✅ 已修复 |
| **F2** | `RefundsTable.php:41`     | `EditAction` 存在但无 edit 页面    | ✅ 已修复 |
| **F3** | `Configure.php`           | 缺少 `auto_complete_days` 联动验证 | ✅ 已修复 |
| **F4** | `StatsOverview.php`       | 缺少缓存，每次加载执行 6 个查询            | ✅ 已修复 |
| **F5** | `ManageOrders.php`        | Tab badge 6 个独立 count 查询     | ✅ 已修复 |
...
**问题**: `cancel()` 中 `OrderCanceled::dispatch` 在 `$order->save()` 之前调用，事件监听器读取数据库状态会不一致。
...
**问题**: `delete()` 复用 `cancel` 的状态验证逻辑，已完成订单无法删除。

**修复**: 添加独立的状态检查，仅允许删除已取消或已完成的订单。
...
**问题**: `deliver()` 中 `$express` 实际是 `OrderShipping` 类型，与快递公司 ID 混淆。

**修复**: 变量名 `$express` → `$shipping`。

#### C4/C5 — 关联名称不匹配

**问题**: Filament 使用 `'expresses'` 但 Order 模型定义的是 `shippings()`。

**修复**:

- `ExpressesRelationManager` → `ShippingsRelationManager`
- `OrderInfolist` 中 `expresses.*` → `shippings.*`
- `OrderResource` 注册新的 `ShippingsRelationManager`

#### C6 — 分类关联名称错误

**问题**: `ProductsTable` 使用 `categories.name` 但 Product 关联是单数 `category()`。

**修复**: `categories.name` → `category.name`

#### C7 — SQL 注入风险

**问题**: `$request->keyword` 直接拼接到 LIKE 查询，`%` 和 `_` 通配符未转义。

**修复**: 使用 `addcslashes` 转义 LIKE 通配符。
...
**问题**: `Sku::find()` 可能返回 null，传入 `OrderItemDto::make()` 会报错。

**修复**: 添加 null 检查并抛出明确异常。
...
**问题**: `ProductCollection` 引用 `$item->sales`，应为 `$item->total_sale`。

**修复**: `sales` → `total_sale`

#### N1-N3 — N+1 查询优化

**问题**: Product 的 `total_sale` 访问器每次调用 `$this->skus()->sum('sale')`，列表场景产生 N+1。

**修复方案**:

1. **访问器优化** — 优先使用 `withSum` 预加载的聚合值：
...
public function getTotalSaleAttribute(): int
{
    ...
    return $this->skus()->sum('sale'); // 降级：单条记录时仍可用
}
```

2. **查询预加载** — 在查询时添加 `withSum`：

```php
Product::ofUp()
    ->with(['brand', 'category', 'storeConfigure'])
    ->withSum('skus', 'sale')  // 新增
    ->paginate();
```

3. **Filament 资源** — 重写 `getEloquentQuery()`：
...
**影响文件**: `Product.php`, `ProductController.php`, `IndexController.php`, `Backend/ProductResource.php`, `Tenant/ProductResource.php`

### 4.2 2026-06-16 修复详情（第二批）

#### S1/S2 — 退款服务状态检查和事件分发

**问题**: `refunded()` 缺少状态验证，重复调用会重复回退库存；缺少退款完成/失败事件分发。

**修复**:

1. 添加状态检查，仅允许 Pending/Processing 状态处理
2. 创建 `RefundCompleted` 和 `RefundFailed` 事件
3. 在处理成功/失败时触发相应事件
...
public function refunded(Refund $refund, bool $result, ...): void
{
    if (!in_array($refund->status, self::PROCESSABLE_STATUSES, true)) {
        throw new RuntimeException('退款单当前状态不可处理');
    }
    // ...
}
```
...
**问题**: `addItem()` 和 `updateItemQty()` 缺少商品上架状态检查。
...
**问题**: `audit()`、`up()`、`down()` 缺少状态转换验证。

**修复**: 添加状态转换规则

```php
// audit: Pending → Up/Rejected
// up: Down/Rejected → Up
// down: Up → Down
```
...
**问题**: `saved` 事件会记录 created 事件，日志包含不必要的初始数据。

**修复**: 改用 `updated` 事件，并检查是否有实际变更
...
**问题**: `saving` 事件每次保存都触发查询，即使 `is_default` 未变化。
...
**问题**: `mergeSessionCart()` 缺少事务保护。

**修复**: 使用 `DB::transaction` 包裹

#### B6 — Eloquent 属性检查

**问题**: `property_exists()` 不可靠，应使用 `getAttribute()`。

**修复**: 改用 `$product->getAttribute('weight') !== null`

#### B8 — 清空购物车优化

**问题**: `clear()` 后重新加载空 items，无意义查询。

**修复**: 使用 `$cart->fresh()` 替代 `$cart->load([...])`

#### B9 — 除零保护

**问题**: `ceilDivision()` 除数为 0 时会报错。
...
**影响方法**: `pay()`, `sign()`, `complete()`, `preparing()`

#### Q1 — OrderCollection 空实现

**问题**: `toArray()` 返回空数组，未完成实现。

**修复**: 删除重写，使用父类 `BaseCollection` 的实现

#### Q3 — CartItem 缺少 product_id

**问题**: CartItem 创建时缺少 `product_id` 字段。

**修复**: 添加 `product_id => $sku->product_id`

#### Q5 — 中英文混用

**问题**: RefundInfolist 标签中英文混用。

**修复**: `'User'` → `'用户'`

#### Q6 — RefundItem 类型转换

**问题**: RefundItem 缺少 `qty` 属性类型定义。

**修复**: 添加 `'qty' => 'integer'` 到 `$casts`

#### Q7 — 全局 Scope 移除方法

**问题**: ProductCategory 全局 scope 无移除方法。

**修复**: 添加 `withoutProductScope()` 方法
...
**问题**: 规则匹配不支持降级（district → city → province）。
...
| #      | 文件                                | 问题描述                         | 修复方式                              |
|--------|-----------------------------------|------------------------------|-----------------------------------|
| **L2** | `OrderService.php:449`            | `deleteExpress()` 缺少状态校验     | 添加 `assertCan()` 状态验证             |
| **L4** | `Refund.php:76-78`                | `refunded()` 空实现             | 移至 `RefundService::refunded()` 实现 |
| **L6** | `IndexController.php`             | `brands()` 和 `banners()` 空实现 | 实现查询逻辑并返回数据                       |
| **L9** | `RefundService.php`               | 缺少退款回调逻辑                     | 实现 `refunded()` 和 `failed()` 回调   |
| -      | `Chain/ContractController.php`    | 缺少部署状态检查                     | 添加 `deploy_status` 验证             |
| -      | `Chain/CertificateController.php` | 缺少禁用状态检查                     | 添加 `isDisabled()` 验证              |
...
| 13 | Q2 — OrderItemDto::getFreight() | 未完成功能  | ⚠️ |
...
1. **Filament 关联名称不匹配**（C4-C6）：`expresses` 应为 `shippings`，`categories` 应为 `category` — ✅ 已修复
2. **N+1 查询**（N1-N3）：Product accessor 触发大量额外查询 — ✅ 已修复（使用 `withSum` 预加载）
3. **状态检查缺失**（S1-S7）：退款重复处理、商品状态未校验 — ✅ 已修复
4. **竞态条件**（B10）：assertCan 在事务外执行 — ✅ 已修复（使用悲观锁）
5. **代码质量**（Q1-Q8）：大部分已修复，Q2/Q4 待处理
